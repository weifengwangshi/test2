...
import com.basefrm.excel.util.ExcelUtil;
...
public abstract class AbstractExcelResolver implements CellValueConverter{
	...
	protected String resolverExpression(String value, String format, Type type,FieldTitleValue fieldValue,int rowNum) {
		try {
			String[] expressions = StringUtils.split(format, ",");
			for (String expression : expressions) {
				String[] val = StringUtils.split(expression, ":");
	...
	protected String getErrorMsg(FieldTitleValue fieldValue, String errMsg, int rowNum){
		StringBuilder err = new StringBuilder();
		err.append("第[").append(rowNum).append("行],[")
				.append(fieldValue.getTitle()).append("]").append(errMsg);
		return err.toString();
