...
public class FieldValue {
	...
	/**纵表时数据所在列数*/
	private String  cols ;
	/**行合并时数据向上读取,默认行不合并(NO/YES)*/
	private String rowsMerge = "NO";
	/**调用函数(PROCEDURE)还是调用过程（FUNCTION）*/
	private String callType;
	/**数据处理类型 --翻译(REPLACE)检查(CHECK)处理(PROCESS)*/
	private String  processType;
	/**传函数名称及参数*/
	private String cellValueConfig;
	/**列合并时数据向左读取，默认列不合并（NO/YES）*/
	private String colsMerge = "NO" ;
	//导入Excel时生效
	/** 是否可以为null */
	...
	public String getCellValueConfig() {
		return cellValueConfig;
	}

	public void setCellValueConfig(String cellValueConfig) {
		this.cellValueConfig = cellValueConfig;
	}

	public String getColsMerge() {
		return colsMerge;
	}

	public void setColsMerge(String colsMerge) {
