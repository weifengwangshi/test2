...
//		return listBean;
		int rowNum = sheet.getLastRowNum();
		//约定数据内容第一列不能空
		for(int ck = rowNum;ck > 0 ;ck--){
			Row   nowrow = sheet.getRow(ck);
				Cell ceck = nowrow.getCell(0);
				if(getCellValue(ceck) == null ){
					rowNum = rowNum -1;
//					if(rowNum == excelDefinition.getTitleIndex()){
//						break;
//					}
				}else{
					break;
		...
		List<T> listBean = new ArrayList<T>(totalNum);
		int loop = 1;
		retyl:	for (int i = titleIndex+1; i <= rowNum; i++) {
			Row row = sheet.getRow(i);
			...
			for (int j = cols; j < cols+titles.size(); j++) {
				if (title.equals(titles.get(j))) {
					try {
						...
						//数据为空时向左读取
						if(value == null && "YES".equals(fieldValue.getColsMerge())){
                             for(int cls = 1;cls < j;cls++){
                             	int jcls = j - cls;
								 Cell cellc = row.getCell(jcls);
								 //获取Excel原生value值
								  value = getCellValue(cellc);
								  if(value != null){
								  	   break;
								  }
							 }
